...
# 1. Create in mysuperpowers location (lowest priority)
mkdir -p "$MYSUPERPOWERS_SKILLS_DIR/priority-test"
cat > "$MYSUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: MySuperPowers version of priority test skill
---
# Priority Test Skill (MySuperPowers Version)

This is the MYSUPERPOWERS version of the priority test skill.

PRIORITY_MARKER_MYSUPERPOWERS_VERSION
EOF
...
if [ -f "$MYSUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] MySuperPowers version exists"
else
    echo "  [FAIL] MySuperPowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled mysuperpowers priority
echo ""
echo "Test 2: Documenting personal vs mysuperpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled mysuperpowers version instead of personal version"

# Test 3: Document project vs bundled mysuperpowers priority
echo ""
echo "Test 3: Documenting project vs personal/mysuperpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled mysuperpowers version instead of project version"

# Test 4: Test a non-colliding bundled mysuperpowers skill is still available
echo ""
echo "Test 4: Testing non-colliding mysuperpowers skill remains available..."

mkdir -p "$MYSUPERPOWERS_SKILLS_DIR/mysuperpowers-only-test"
cat > "$MYSUPERPOWERS_SKILLS_DIR/mysuperpowers-only-test/SKILL.md" <<'EOF'
---
name: mysuperpowers-only-test
description: MySuperPowers-only priority test skill
---
# MySuperPowers Only Test Skill

PRIORITY_MARKER_MYSUPERPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"mysuperpowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_MYSUPERPOWERS_ONLY_VERSION" "Non-colliding mysuperpowers skill is still registered"

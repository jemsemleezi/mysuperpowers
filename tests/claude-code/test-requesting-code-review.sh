...
echo "  2. Adding a second commit that plants two real bugs"
echo "  3. Dispatching the code reviewer via the requesting-code-review skill"
echo "  4. Verifying the reviewer flags the planted bugs at Critical/Important severity"
echo ""
...
PROMPT='
I just finished a refactor. The change is between commits '$BASE_SHA' and '$HEAD_SHA' on the current branch.

Use the mysuperpowers:requesting-code-review skill to review these changes before I merge. Follow the skill exactly: dispatch the code reviewer subagent with the template, give the subagent the SHA range, and report back what it found.

Print the reviewers full output.
'

# Run claude from inside the test project
...
if [ -z "$SESSION_FILE" ] || [ ! -f "$SESSION_FILE" ]; then
    ...
    FAILED=$((FAILED + 1))
elif ! grep -q '"name":"Skill".*"skill":"requesting-code-review"' "$SESSION_FILE"; then
...
# Test 3: Reviewer caught the credential / password issue
...
# Test 4: Reviewer marked at least one issue as Critical or Important

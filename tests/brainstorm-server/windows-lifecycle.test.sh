...
# Requirements:
#   - Node.js in PATH
#   - Run from the repository root, or set MYSUPERPOWERS_ROOT
...
REPO_ROOT="${MYSUPERPOWERS_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
START_SCRIPT="$REPO_ROOT/skills/engineering/brainstorming/scripts/start-server.sh"
STOP_SCRIPT="$REPO_ROOT/skills/engineering/brainstorming/scripts/stop-server.sh"
SERVER_JS="$REPO_ROOT/skills/engineering/brainstorming/scripts/server.cjs"
...
FAKENODE
  ...
  if [[ -z "$owner_pid_value" || "$owner_pid_value" == "__UNSET__" ]]; then

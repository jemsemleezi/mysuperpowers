---
name: tdd
description: Test-driven development with red-green-refactor loop. Use when implementing features or bugfixes, before writing implementation code.
---
...
┌─────────────────────────────────────────────────────┐
│                                                     │
│   RED ──► Verify RED ──► GREEN ──► Verify GREEN ──► REFACTOR ──► Next ──► RED
│   │          │             │            │               │
│   └─ wrong ──┘             └── no ──────┘               │
│                                    ▲ └── stay green ────┘
│                                    │
...
Write **one** minimal test. One behavior per test.
...
Don't add features, refactor other code, or "improve" beyond the test. Don't anticipate future tests.
...
Ask the user: "What should the public interface look like? Which behaviors are most important to test?"
...
| **Resilient** | Survives internal refactor | Breaks when renaming internals |
| **Integration-style** | Exercises real code paths | Mocks internal collaborators |
...
| "Writing all tests first is more efficient" | Horizontal slices produce brittle tests. Vertical slice instead. |
...
2. **Return results, don't produce side effects**
   ```typescript
   // Testable: returns value, easy to assert
